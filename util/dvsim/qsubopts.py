...
class qsubOptions():
    ...
    def __init__(self, optstring = '', prog = 'qsub'):
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin']:
            self.parser.add_argument('-@', metavar = 'optionfile', help = """\
        ...
        if prog in ['qsub', 'qalter']:
            self.parser.add_argument('-a', metavar = 'date_time', help = """\
        ...
        if prog in ['qsub', 'qalter', 'qrsh', 'qsh', 'qlogin']:
            self.parser.add_argument('-ar', metavar = 'ar_id', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-A', metavar = 'account_string', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        self.parser.add_argument('-binding', nargs='+',
                                 metavar=('binding_instance', 'binding_strategy'), help = """\
       -binding [ binding_instance ] binding_strategy
        ...
        if prog in ['qsub', 'qrsh']:
            self.parser.add_argument('-b', choices = yesno, help = """\
        ...
        if prog in ['qsub', 'qalter']:
            self.parser.add_argument('-c', metavar = 'occasion_specifier', help = """\
              Available for qsub and qalter only.
        ...
        if prog in ['qsub', 'qalter']:
            self.parser.add_argument('-ckpt', metavar = 'ckpt_name', help = """\
              Available for qsub and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin']:
            self.parser.add_argument('-clear', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qalter']:
            self.parser.add_argument('-cwd', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qrsh']:
            self.parser.add_argument('-C', metavar = 'prefix_string', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-dc', action = 'append', metavar = 'variable', help = """\
       -dc variable,...
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsh', 'qrsh']:
            self.parser.add_argument('-display', metavar = 'display_specifier', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-dl', metavar = 'date_time', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-e', metavar = 'path', help = """\
          -e [[hostname]:]path,...
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-hard', action = 'store_true', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qrsh', 'qalter', 'qresub']:
            ...
            self.parser.add_argument('-hold', choices = 'usonUOS', help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qalter']:
            self.parser.add_argument('-hold_jid', nargs = '+', metavar = 'wc_job_list', help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qalter']:
            self.parser.add_argument('-hold_jid_ad', nargs = '+',
                                     metavar = 'wc_job_list', help = """\
              Available for qsub, qrsh, and qalter only. See sge_types(1).
              for wc_job_list definition.
        ...
        if prog in ['qsub', 'qalter']:
            self.parser.add_argument('-i', metavar = 'file', help = """\
       -i [[hostname]:]file,...
              Available for qsub, and qalter only.
        ...
        if prog in ['qrsh', 'qmake']:
            self.parser.add_argument('-inherit', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-j', choices = yesno, help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin']:
            self.parser.add_argument('-jsv', metavar = 'jsv_url', help = """\
              Available for qsub, qsh, qrsh and qlogin only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-l', metavar = 'keywords', help = """\
       -l resource=value,...
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            ...
            self.parser.add_argument('-m', nargs = '+', choices = 'beasn', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-M', metavar = 'user[@host]', help = """\
       -M user[@host],...
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-masterq', nargs='+', metavar='wc_queue_list', help="""\
        ...
        if prog in ['qsub', 'qrsh', 'qalter']:
            self.parser.add_argument('-notify', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin']:
            self.parser.add_argument('-now', choices = yesno, help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-N', metavar = 'name', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qrsh']:
            self.parser.add_argument('-noshell', action = 'store_true', help = """\
        ...
        if prog in ['qrsh']:
            self.parser.add_argument('-nostdin', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-o', metavar = 'path', help = """\
       -o [[hostname]:]path,...
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qalter']:
            self.parser.add_argument('-ot', metavar = 'override_tickets', help = """\
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-P', metavar = 'project_name', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-p', metavar = 'priority', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub', 'qsh', 'qrsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-slot', metavar = 'slot', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
       ...
       -pe parallel_environment n[-[m]]|[-]m,...
        ...
        if prog in ['qrsh', 'qlogin']:
            self.parser.add_argument('-pty', choices = yesno, help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-q', nargs = '+', metavar = 'wc_queue_list', help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-R', choices = yesno, help = """\
              Available for qsub, qrsh, qsh, qlogin and qalter.
       ...
       -sc variable[=value],...
        ...
        if prog in ['qsub']:
            self.parser.add_argument('-shell', choices = yesno, help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-soft', action = 'store_true', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter only.
        ...
        if prog in ['qsub']:
            self.parser.add_argument('-sync', choices = yesno, help = """\
              Available for qsub.
        ...
        if prog in ['qsub', 'qsh', 'qalter']:
            self.parser.add_argument('-S', metavar = 'pathname', help = """\
       -S [[hostname]:]pathname,...
        ...
        if True or prog in ['qsub', 'qalter']:
            self.parser.add_argument('-t', metavar = 'n[-m[:s]]', help = """\
              Available for qsub and qalter only.
        ...
        if prog in ['qsub', 'qalter']:
            self.parser.add_argument('-tc', type = int, metavar = 'max_running_tasks', help = """\
        ...
        if prog in ['qsub']:
            self.parser.add_argument('-terse', action = 'store_true', help = """\
        ...
        if prog in ['qalter']:
            self.parser.add_argument('-u', metavar = 'username', help = """\
       -u username,...
        ...
        if prog in ['qsub', 'qrsh', 'qalter']:
            self.parser.add_argument('-v', metavar = 'variable[=value]', help = """\
       -v variable[=value],...
        ...
        if prog in ['qrsh', 'qmake']:
            self.parser.add_argument('-verbose', action = 'store_true', help = """\
              Available only for qrsh and qmake(1).
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-verify', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            ...
            self.parser.add_argument('-V', action = 'store_true', help = """\
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qlogin', 'qalter']:
            self.parser.add_argument('-w', choices = 'ewnpv', help = """\
              Available for qsub, qsh, qrsh, qlogin and qalter.
        ...
        if prog in ['qsub', 'qrsh', 'qsh', 'qalter']:
            self.parser.add_argument('-wd', metavar = 'working_dir', help = """\
              Available for qsub, qsh, qrsh and qalter only.
        ...
        if prog in ['qsub', 'qrsh', 'qalter']:
            self.parser.add_argument('command_args', nargs = '*', help = """\
        ...
        if prog in ['qsh']:
            self.parser.add_argument('xterm_args', nargs = '*', help = """\
...
#        exestring = exestring.replace('-j y','')
        ...
        if mode == 'echo':
            ...
            import subprocess
            p = subprocess.Popen(exestring, stdout = subprocess.PIPE,
